...
const DB_PATH = path.join(__dirname, 'test_integration.db');
...
function doCheckout(customerId: number, cart: any[], paymentMode: string, amountPaid: number) {
  const checkoutTx = db.transaction(() => {
    ...
    cart.forEach(item => {
      if (item.instance_id) {
        const inst = db.prepare('SELECT purchase_cost FROM product_instances WHERE instance_id = ?').get(item.instance_id) as any;
        ...
        const prod = db.prepare('SELECT purchase_cost FROM products WHERE product_id = ?').get(item.product_id) as any;
...
runTest('B1 - Checkout transaction commits', () => {
  ...
  const inst = db.prepare("SELECT status, warranty_expires_at FROM product_instances WHERE instance_id = 1").get() as any;
  if (inst.status !== 'SOLD' || !inst.warranty_expires_at) throw new Error("Serial not marked SOLD or warranty not set");

  const ledger = db.prepare("SELECT * FROM customer_ledger WHERE customer_id = 3 AND type = 'SALE'").get() as any;
  if (!ledger || ledger.amount !== 11800) throw new Error("Customer ledger not written correctly");

  const seq = db.prepare("SELECT value FROM settings WHERE key = 'next_invoice_no'").get() as any;
  if (seq.value !== '1002') throw new Error("Invoice sequence not incremented");
});

runTest('B2 - Checkout rolls back atomically', () => {
  const beforeSeq = (db.prepare("SELECT value FROM settings WHERE key = 'next_invoice_no'").get() as any).value;
  try {
    doCheckout(1, [
    ...
    // Assert rolled back
    const inst = db.prepare("SELECT status FROM product_instances WHERE instance_id = 2").get() as any;
    if (inst.status !== 'IN_STOCK') throw new Error("Serial 2 was not rolled back to IN_STOCK");
    
    const afterSeq = (db.prepare("SELECT value FROM settings WHERE key = 'next_invoice_no'").get() as any).value;
    if (afterSeq !== beforeSeq) throw new Error("Invoice sequence incremented during failed transaction");
  }
});
...
runTest('B3 - Real UNIQUE constraint', () => {
  try {
    ...
    if (e.message === "Should have failed") throw e;
    if (!e.message.includes('UNIQUE constraint failed')) throw new Error(`Unexpected error: ${e.message}`);
  }
});

runTest('B4 - Persisted GST split', () => {
  const saleIdIntra = doCheckout(1, [{ product_id: 2, price: 11800, qty: 1 }], 'CASH', 11800);
  const intra = db.prepare("SELECT cgst, sgst, igst FROM sales WHERE sale_id = ?").get(saleIdIntra) as any;
  if (intra.igst !== 0 || intra.cgst === 0 || intra.sgst === 0) throw new Error("Intra-state split wrong");

  const saleIdInter = doCheckout(2, [{ product_id: 2, price: 11800, qty: 1 }], 'CASH', 11800);
  const inter = db.prepare("SELECT cgst, sgst, igst FROM sales WHERE sale_id = ?").get(saleIdInter) as any;
  if (inter.igst === 0 || inter.cgst !== 0 || inter.sgst !== 0) throw new Error("Inter-state split wrong");
});

runTest('B5 - Print render data and numToWords verification', () => {
  const interSale = db.prepare("SELECT * FROM sales WHERE customer_id = 2").get() as any;
  if (interSale.igst === 0) throw new Error("Inter-state sale IGST should be > 0");
  
  const intraSale = db.prepare("SELECT * FROM sales WHERE customer_id = 1").get() as any;
  if (intraSale.cgst === 0) throw new Error("Intra-state sale CGST should be > 0");

  const items = db.prepare(`
    SELECT si.*, p.gst_rate FROM sale_items si JOIN products p ON si.product_id = p.product_id WHERE si.sale_id = ?
  `).all(interSale.sale_id) as any[];

  if (items[0].gst_rate !== 18) throw new Error("Per-line gst_rate not fetched correctly");
  ...
  const numToWords = (numInPaise: number) => {
    ...
    if (result === '') return 'Zero Rupees Only';
    return result + ' Only';
  };

  if (numToWords(500000) !== 'Five Thousand Rupees Only') throw new Error(`numToWords 500000: ${numToWords(500000)}`);
  if (numToWords(11850) !== 'One Hundred Eighteen Rupees and Fifty Paise Only') throw new Error(`numToWords 11850: ${numToWords(11850)}`);
  if (numToWords(0) !== 'Zero Rupees Only') throw new Error(`numToWords 0: ${numToWords(0)}`);
});

// --- REPORTS R1-R5 TESTS ---
runTest('R1 - GST reconciliation', () => {
  const invoices = db.prepare(`SELECT * FROM sales WHERE status != 'CANCELLED'`).all() as any[];
  let totalCgst = 0, totalSgst = 0, totalIgst = 0;
  invoices.forEach((i: any) => { totalCgst += i.cgst; totalSgst += i.sgst; totalIgst += i.igst; });
  
  const rawSum = db.prepare(`SELECT SUM(cgst) as c, SUM(sgst) as s, SUM(igst) as i FROM sales WHERE status != 'CANCELLED'`).get() as any;
  if (totalCgst !== (rawSum.c || 0)) throw new Error("CGST mismatch");
  if (totalSgst !== (rawSum.s || 0)) throw new Error("SGST mismatch");
  if (totalIgst !== (rawSum.i || 0)) throw new Error("IGST mismatch");
});
...
runTest('R2 - Margin math', () => {
  ...
  const testItem = db.prepare(`SELECT unit_cost FROM sale_items LIMIT 1`).get() as any;
  if (testItem.unit_cost === 0 && marginData.revenue > 0) throw new Error("unit_cost was not captured");
  
  if (marginData.revenue - marginData.cogs !== marginData.profit) throw new Error("Margin math failed");
});
...
runTest('R3 - Low-stock', () => {
  ...
  if (!prod1 || prod1.in_stock_qty !== 1) throw new Error("Product 1 should be low stock with qty 1");
  
  const prod2 = lowStock.find(p => p.product_id === 2);
  if (prod2) throw new Error("Product 2 should NOT be low stock");
});
...
runTest('R4 - Stock valuation', () => {
  ...
  if (valuation.serialized_value !== 8000) throw new Error(`Serialized value expected 8000, got ${valuation.serialized_value}`);
  if (valuation.loose_value !== 5000) throw new Error(`Loose value expected 5000, got ${valuation.loose_value}`);
});
...
runTest('R5 - Date filter', () => {
  ...
  const salesInRange = db.prepare(`SELECT COUNT(*) as c FROM sales WHERE created_at >= ? AND created_at <= ?`).get(startDate, endDate) as any;
  const allSales = db.prepare(`SELECT COUNT(*) as c FROM sales`).get() as any;
  
  if (salesInRange.c === allSales.c) throw new Error("Date filter did not exclude the old sale");
});
